...
public class GameController {
	...
	public void beginStatus() {
		...
		for (RFMPlayer p : hunterList) {
			...
			if (hunter != null) {
				hunter.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, 30 * TPS, 1));
			}
		}

		// set time out
		runForMoney.getServer().getScheduler().scheduleSyncDelayedTask(runForMoney, new Runnable() {
	...
	public void blindStatus() {
		...
		if (chance > r.nextDouble()) {
			...
			for (RFMPlayer p : hunterList) {
				Player hunter = (Bukkit.getServer().getPlayer(p.getName()));
				if (hunter != null) {
					hunter.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 30 * TPS, 1));
				}
			}
			...
			for (RFMPlayer p : runnerList) {
				...
				if (runner != null) {
					runner.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 30 * TPS, 1));
				}
			}
	...
	private void checkAllPlayerBeforeStart() {
		...
		for (RFMPlayer rfmPlayer : runnerList) {
			...
			if (player == null) {
				...
				broadCastInGame(ChatColor.AQUA + rfmPlayer.getName() + " 遊戲開始時不在遊戲區內，判定放棄遊戲!" + ChatColor.RESET);
			}
		}
	...
	public void checkPlayerDeath(Player player) {
		...
		if (rfmRunner != null && rfmRunner.isAlive()) {
			...
			broadCastInGame(ChatColor.RED + "逃亡者  " + player.getName() + " 事故身亡！！！ (存活人數剩" + getRunnerAlive() + " 人)\n" + getTime());
		...
		if (rfmHunter != null && rfmHunter.isAlive()) {
			...
			broadCastInGame(ChatColor.RED + "獵人  " + player.getName() + " 事故身亡！！！ (存活人數剩" + getHunterAlive() + " 人)\n" + getTime());
	...
	public void checkPlayerQuit(Player player) {
		...
		// send to observer
		if ((rfmPlayer != null) || (player != null && player.getWorld() != null && player.getWorld().getName().equalsIgnoreCase(Constants.ARENA_WORLD_NAME))) {
			// teleport to observer portal
			runForMoney.getTeleporter().moveToPortal(player, "RFMobserver");
		}
		...
		if (rfmPlayer != null) {
			...
			if (rfmPlayer.isAlive()) {
				rfmPlayer.setAlive(false);
				broadCastInGame(ChatColor.AQUA + rfmPlayer.getName() + "(" + type + ")" + " 放棄了遊戲!" + ChatColor.RESET);
	...
	public void checkRunterKillsRunner(Player hunter, Player runner) {
		...
		if (rfmRunner != null && rfmHunter != null && rfmRunner.isAlive() && rfmHunter.isAlive()) {
			rfmRunner.setAlive(false);
			rfmHunter.addKills();
			...
			broadCastInGame(ChatColor.LIGHT_PURPLE + "獵人 " + hunter.getName() + " 抓到了逃亡者  " + runner.getName() + " ！！！ (存活人數剩" + getRunnerAlive() + " 人)\n"
					+ getTime());
			checkIfGameover();
		}
	}
	...
	public String getStatus() {
		...
		// prepare runner
		statusString.append(ChatColor.AQUA + "參賽者 (共 " + runnerList.size() + "人/存活人數" + getRunnerAlive() + ") ");
		...
		statusString.append(ChatColor.GREEN + "獵人 (共 " + hunterList.size() + "人) ");
		...
		for (RFMPlayer p : hunterList) {
			...
			statusString.append(color + p.getName() + "(" + p.getKills() + ") " + ChatColor.RESET + ", ");
		}
	...
	public String getTime() {
		...
		if (gameStatus == GameStatus.Running) {
			...
			statusString.append("遊戲時間已經過: " + ChatUtil.secToString(gameTime) + ", 尚有: " + ChatUtil.secToString(totalTime - gameTime));
	...
	public void lastStatus() {
		...
		for (RFMPlayer p : hunterList) {
			Player hunter = (Bukkit.getServer().getPlayer(p.getName()));
			if (hunter != null) {
				hunter.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 30 * TPS, 1));
			}
		}
	...
	public void start() {
		...
		Bukkit.getPluginManager().registerEvents(new AttackEvent(this), runForMoney);

		Bukkit.getPluginManager().registerEvents(new CommandEvent(this), runForMoney);

		Bukkit.getPluginManager().registerEvents(new QuitJoinEvent(this), runForMoney);

		Bukkit.getPluginManager().registerEvents(new PlayerDeadEvent(this), runForMoney);

		Bukkit.getPluginManager().registerEvents(new InventoryEventManager(this), runForMoney);
		...
		runForMoney.getServer().getScheduler().runTaskTimerAsynchronously(runForMoney, new Runnable() {
